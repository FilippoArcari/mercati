...
def make_windows(df_x, horizon, sliding_windows, stride, n_target_cols):
    ...
    y_valid = torch.stack([
        X_tensor[i * stride + sliding_windows : i * stride + sliding_windows + horizon,
                 :n_target_cols]
...
def my_app(cfg: DictConfig) -> None:
    ...
    df, n_context_features, scaler = get_data(cfg.data.tickers, cfg.data.start_date, cfg.data.end_date, cfg.frequency, cfg.data.inflation_series, cfg.data.split_train)
    ...
    safe_config["prediction"]["real_features"] = safe_config['prediction']['n_features'] - safe_config['prediction']['n_context_features']
    predictor_config = safe_config["prediction"]

    # ── FETCH DATA AND SPLIT DATASET ───────────────────────────────────────────
    rows = df.shape[0]
    ...
    if cfg.step == "optimize":
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            # hidden_size deve essere divisibile per num_heads
            ...
            # Il callback legge val_loss dopo ogni epoch e dice al pruner se tagliare
        ...
        if pruner_name == "median":
            ...
            pruner = optuna.pruners.HyperbandPruner(
                min_resource=3, max_resource=max_epochs, reduction_factor=3
            )
        else:
            raise ValueError(f"Pruner sconosciuto: '{pruner_name}'. Scegli 'median' o 'hyperband'.")
    ...
    # ── TRADE ──────────────────────────────────────────────────────────────────
        ...
        trader = PPOTrader(train_env, safe_config['trader'])
        print("Starting training...")
        trader.train()
        print("Training completed. Starting test...")
        # trader.test(test_env, render=False)

        trader.test(test_env, render=True)
        print("Test completed.")
        history = test_env.get_history()
        
        # Get rows where a trade happened
        trade_rows = history[history['actions'].apply(lambda acts: any(a.get("type") in ("buy", "sell") for a in acts.values()))]
        ...
        print(f"\nOperazioni totali : {len(trade_rows)} | Shares (volume) bought: {final_bought[final_bought > 0].to_dict()} | Shares (volume) sold: {final_sold[final_sold > 0].to_dict()}")
        ...
        history.plot(x="step", y="portfolio_value", title="Valore portfolio nel tempo")
        history.plot(x="step", y="portfolio_value", title="Prezzo nel tempo")
    ...
    # ── TRAIN ──────────────────────────────────────────────────────────────────
        ...
        checkpoint_callback = ModelCheckpoint(dirpath="checkpoints", filename=checkpoint_name, save_top_k=1, verbose=True, monitor="val_loss", mode="min", save_last=True, enable_version_counter=False)
        trainer = L.Trainer(max_epochs=cfg.prediction.epochs, callbacks=[early_stop_callback, checkpoint_callback], accelerator="cuda" if torch.cuda.is_available() else "cpu", gradient_clip_val=1.0, log_every_n_steps=10)
    ...
    # ── TEST ───────────────────────────────────────────────────────────────────
        ...
        test_loader = DataLoader(test_dataset, batch_size=cfg.prediction.batch_size, shuffle=False, num_workers=NUM_WORKERS)
        ...
        columns   = pd.MultiIndex.from_product(
            [range(1, horizon + 1), columns_name],
            names=["step", "ticker"]
        )
        ...
        real_rescaled = scaler.inverse_transform(
            np.pad(price_real.values.reshape(-1, n_main), ((0, 0), (0, predictor_config['n_context_features'])), mode='constant')
        )[:, :n_main]
